...
import { Doador, StatusDoador, Endereco } from './types';
import { MOCK_DOADORES } from './mockData';
...
function StatusBadge({ status }: { status: StatusDoador }) {
...
const blankDoador = (): Omit<Doador, 'id'> => ({
  nome: '',
  email: '',
  status: 'ativo',
  cpfCnpj: '',
...
// ════════════════════════════════════════════════════════════════
function DoadorForm({
  doador, mode, onBack, onSave,
}: {
  doador: Doador | null;
  mode: 'new' | 'edit' | 'view';
  onBack: () => void;
  onSave: (d: Doador) => void;
}) {
  const ro = mode === 'view';
  const [form, setForm] = useState<Doador>(
    doador ?? { id: Date.now(), ...blankDoador() }
  );
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [viewMode, setViewMode] = useState(mode);

  const f = <K extends keyof Doador>(k: K) => (v: Doador[K]) => setForm(prev => ({ ...prev, [k]: v }));
  ...
  return (
    <form onSubmit={handleSave} className="flex flex-col h-full bg-[#f8fafd]">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-[#1a2744]">
              {viewMode === 'new' ? 'Novo Doador' : viewMode === 'edit' ? 'Editar Doador' : 'Visualizar Doador'}
            </h2>
            <p className="text-xs text-[#6b7a9e] mt-1">Formulário de dados cadastrais do doador</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-[#f8fafd]">
          <Section title="Informações do Doador">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <select disabled={ro} className={ro ? inpRO : inp} value={form.status} onChange={e => f('status')(e.target.value as StatusDoador)}>
...
//  DOADORES PAGE (LIST)
// ════════════════════════════════════════════════════════════════
export function DoadoresPage({ initialView }: { initialView?: 'list' | 'form' }) {
  const [doadores, setDoadores] = useState<Doador[]>(MOCK_DOADORES);
  const [view, setView] = useState<'list' | 'form'>(initialView ?? 'list');
  const [selected, setSelected] = useState<Doador | null>(null);
  const [formMode, setFormMode] = useState<'new' | 'edit' | 'view'>(initialView === 'form' ? 'new' : 'new');
  const [search, setSearch] = useState('');
  const [filterStatus, setFilterStatus] = useState<StatusDoador | 'ALL'>('ALL');
  ...
  const filtered = useMemo(() => {
    let r = doadores.filter(d => {
      const q = search.toLowerCase();
      const matchSearch = !q || d.nome.toLowerCase().includes(q) || d.email.toLowerCase().includes(q) || d.cpfCnpj.includes(q);
    ...
    return r;
  }, [doadores, search, filterStatus, sortField, sortDir]);
  ...
  const openForm = (mode: 'new' | 'edit' | 'view', d?: Doador) => {
    setSelected(d ?? null); setFormMode(mode); setView('form');
  };

  const handleSave = (d: Doador) => {
    setDoadores(prev => {
  ...
  const handleToggleStatus = (id: number, currentStatus: StatusDoador) => {
    setConfirmId(id);
  ...
  const executeToggleStatus = () => {
    ...
    setDoadores(prev => prev.map(d => d.id === confirmId ? { ...d, status: confirmAction === 'inativar' ? 'inativo' : 'ativo' } : d));
    setConfirmId(null);
  };

  if (view === 'form') {
    return <DoadorForm doador={selected} mode={formMode} onBack={() => { setView('list'); setSelected(null); }} onSave={handleSave} />;
  }
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafd]">
      ...
      <div className="flex items-center justify-between px-8 py-5 bg-white border-b border-[#e4e9f4] shrink-0">
        <div>
          <h1 className="text-lg font-bold text-[#1a2744]">Doadores</h1>
          <p className="text-sm text-[#6b7a9e] mt-0.5">{total} {total === 1 ? 'doador encontrado' : 'doadores encontrados'}</p>
        </div>
        ...
        <div className="px-8 py-4 bg-[#f8fafc] border-b border-[#e4e9f4] flex gap-4 shrink-0">
          <div className="flex flex-col gap-1">
            ...
            <select className="rounded-lg border border-[#d0daef] bg-white px-3 py-1.5 text-xs text-[#3d4f72] outline-none focus:border-[#2b65bf]"
              value={filterStatus} onChange={e => { setFilterStatus(e.target.value as StatusDoador | 'ALL'); setPage(0); }}>
          ...
          <div className="flex-1 overflow-auto">
            <table className="w-full border-collapse">
              ...
              <tbody className="divide-y divide-[#e4e9f4]">
                {pageData.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-4 py-8 text-center text-sm font-medium text-[#6b7a9e]">Nenhum doador encontrado.</td>
                  </tr>
                ) : (
                  pageData.map(d => (
                    <tr key={d.id} className="hover:bg-[#f8fafc]/50 transition-colors">
                      <td className={`${tdCls} font-bold`}>{d.nome}</td>
                      <td className={tdCls}>{d.email}</td>
                      <td className={tdCls}>{fmtCPF_CNPJ(d.cpfCnpj)}</td>
                      <td className={tdCls}>{fmtPhone(d.telefone)}</td>
                      <td className={tdCls}><StatusBadge status={d.status} /></td>
                      <td className="px-4 py-3 text-right">
                        <div className="flex gap-1 justify-end">
                          <button onClick={() => openForm('view', d)} title="Visualizar" className="p-1.5 rounded text-[#6b7a9e] hover:text-[#2b65bf] hover:bg-[#f0f4fb] transition-colors"><Eye size={16} /></button>
                          <button onClick={() => openForm('edit', d)} title="Editar" className="p-1.5 rounded text-[#6b7a9e] hover:text-[#2b65bf] hover:bg-[#f0f4fb] transition-colors"><Pencil size={16} /></button>
                          <button onClick={() => handleToggleStatus(d.id, d.status)} title={d.status === 'ativo' ? 'Inativar' : 'Ativar'}
                            className={`p-1.5 rounded transition-colors ${d.status === 'ativo' ? 'text-red-500 hover:bg-red-50' : 'text-green-600 hover:bg-green-50'}`}>
                            {d.status === 'ativo' ? <ToggleRight size={18} /> : <ToggleLeft size={18} />}
                          </button>
                        </div>
                      </td>
                    </tr>
      ...
      {confirmId !== null && (
        <ConfirmDialog
          message={`Tem certeza que deseja ${confirmAction} este doador?`}
